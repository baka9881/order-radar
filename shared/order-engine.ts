...
export const STATUS: Record<Signal, { label: string; action: string; short: string }> = {
  green: { label: "值得接", action: "接", short: "接" },
  yellow: { label: "看情況", action: "看", short: "看" },
  red: { label: "先不要", action: "拒", short: "拒" },
};

export const RETURN_MODES: Record<ReturnMode, { label: string; description: string; multiplier: number }> = {
...
export function calculateOrder(input: OrderInput, settings: CalculatorSettings = DEFAULT_SETTINGS): OrderResult {
  ...
  const greenMinimum = Math.max(45, effectiveDistance * settings.greenPerKm, effectiveDistance * settings.fullCostPerKm + (settings.greenHourly * effectiveMinutes) / 60);
  const yellowMinimum = Math.max(45, effectiveDistance * settings.yellowPerKm, effectiveDistance * settings.fullCostPerKm + (settings.yellowHourly * effectiveMinutes) / 60);

  let signal: Signal = "red";
  if (amount >= greenMinimum) signal = "green";
  else if (amount >= yellowMinimum) signal = "yellow";

  return { signal, effectiveDistance, effectiveMinutes, fuelPerKm, fuelCost, cashNet, fullNet, cashHourly, fullHourly, perKm, greenMinimum, yellowMinimum };
}
...
export function parseOfferText(input: string) {
  ...
  const minutes = hourMatch ? Number(hourMatch[1]) * 60 + Number(hourMatch[2] ?? 0) : minuteMatch ? Number(minuteMatch[1]) : Number.NaN;
  const distances = [...input.matchAll(/(\d+(?:\.\d+)?)\s*(?:公里|公裏|km)/gi)]
    .map((match) => Number(match[1]))
    .filter(Number.isFinite);
  if (!amounts.length || !Number.isFinite(minutes) || !distances.length) return null;
  return { amount: Math.max(...amounts), distance: Math.max(...distances), minutes };
}
